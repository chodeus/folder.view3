...
|---|--------|---------|---------|
| 175 | Custom-action icon (`script_icon`) sanitised to a CSS-class charset before use in the folder context menus (it was inserted unescaped next to the already-escaped action label). | `scripts/docker.js`, `scripts/vm.js`, `scripts/dashboard.js` | 2026.07.07 |
| 174 | GitHub theme import caps per-file download size (2 MB) and file count (200) before writing to the USB boot flash (prevents flash-fill / memory exhaustion). | `server/lib.php` | 2026.07.07 |
| 173 | Container WebUI / Tailscale-WebUI links validated to `http(s)`/relative only via `fv3_safe_http_url()` — a container label can no longer supply a `javascript:` scheme that renders as a clickable link. | `server/lib.php` | 2026.07.07 |
| 172 | Container shell label (`net.unraid.docker.shell`) sanitised to a shell-path charset at the source in `readInfo()`; it is rendered into an inline `onclick` JS-string where HTML-entity escaping is the wrong context. | `server/lib.php` | 2026.07.07 |
| 171 | `importAll` (full-backup restore) allowlists folder-id keys to `^[A-Za-z0-9+/=]+$` and requires each folder to be an object; `updateFolder` also rejects non-object payloads. Prevents a crafted backup from injecting a folder id that renders unescaped into row markup. | `server/lib.php` | 2026.07.07 |
| 170 | `upload_theme.php` folder-upload re-validates the extension on the final on-disk name (not just the multipart part name), realpath-re-confines the target under the styles dir, and caps per-file size/count — closing an arbitrary-extension write to the web-served plugin styles directory. | `server/upload_theme.php` | 2026.07.07 |
| 169 | Revert of #167 — that change broke every legitimate mutation (folder create/edit/delete, CSS save, settings save all returned 403). Root cause: Unraid's global `auto_prepend_file` (`webGui/include/local_prepend.php`) already validates `csrf_token` on every POST and then `unset()`s both `$_POST['csrf_token']` and `$_SERVER['HTTP_X_CSRF_TOKEN']` before the plugin script runs. `fv3_validate_csrf()` then read the (now-empty) token and 403'd. #167's "verified on PHP 8.4" check passed only because it ran under CLI, where `REQUEST_METHOD` is not `POST`, so local_prepend's strip block never executes. Fix: removed the `fv3_validate_csrf()` call (and the dead function) from `fv3_post_init()`; CSRF remains fully enforced by Unraid's global layer (the documented model — "Unraid emhttp auto-validates"). Confirmed in a real browser against the live server. | `server/lib.php` | 2026.06.22 |
| 167 | **Superseded by #169 (reverted in 2026.06.13.2).** Wired `fv3_validate_csrf()` into `fv3_post_init()` believing CSRF was unenforced. This was both redundant (Unraid enforces CSRF globally via local_prepend) and broken (local_prepend strips the token before the plugin sees it → 403 on all legit POSTs). | `server/lib.php` | 2026.06.22 |
| 168 | Theme delete/toggle accepted `entry="."`/`".."` (regex `^[a-zA-Z0-9._-]+$` permits all-dot strings), resolving `$path` to the plugin config dir and recursively deleting `docker.json`, `vm.json`, `settings.json`, `css-config.json` and all themes. Added explicit `.`/`..` rejection in `deleteTheme()` and `toggleTheme()`. | `server/lib.php` | 2026.06.22 |
...
| # | Change | File(s) | Version |
|---|--------|---------|---------|
| 219 | Explicit-beats-regex membership (issue #46): a container explicitly assigned to (or docker-label-claimed by) any folder is subtracted from every other folder's regex matches. Each `createFolders` precomputes the assigned-elsewhere union (explicit `containers` arrays; docker surfaces also add `folder.view3` label claims) and attaches it to the in-memory folder objects; all four regex expansion sites gained the subtraction clause. The folder editor's availability exclusion no longer expands other folders' regexes — only explicit members hide containers, so a container held elsewhere purely by regex stays pickable, and claiming it explicitly automatically removes it from the regex folder. `syncContainerOrder` mirrors this with a pass-1 explicit-members union so autostart grouping matches the UI. A `^` regex folder is now a working "unassigned containers" catch-all. Regex-vs-regex overlap remains first-wins by render order; server-side sync still ignores labels (pre-existing divergence). | `scripts/docker.js`, `scripts/vm.js`, `scripts/dashboard.js`, `scripts/folder.js`, `server/lib.php` | 2026.07.11 |
| 218 | Dashboard "Open WebUI" preview context (extends #41's folder-level option): `dashboard_context` value `3` strips the native tile onclick and opens `ct.info.State.WebUi` in a new tab (`noopener`) when the container has a WebUI; containers without one keep the native menu. Reuses the `context-option-3` i18n keys; both `dashboard_context` allowlists extended. | `FolderView3.page`, `scripts/dashboard.js`, `server/lib.php` | 2026.07.11 |
| 217 | New Preview Context option "Open WebUI" (value 3, issue #41): in the collapsed preview a click on a container's icon opens its WebUI in a new tab (`window.open` with `noopener`; URL is the server-sanitised `ct.info.State.WebUi`). Containers without a WebUI keep Unraid's native context menu as fallback — the four `addPreview` closures take a `webui` flag computed at the call site, so the native inline onclick is only stripped when a WebUI exists. No running-state gate (globe-icon parity). Offered per-folder and as a global default (`default_context` allowlists extended with `'3'` in both `updateSettings` and `updateSettingsBatch`). Docker page only — the Dashboard has no collapsed preview and VM folders never see the docker-gated option. Older plugin versions reading `context: 3` degrade gracefully to Default behaviour. | `Folder.page`, `FolderView3.page`, `scripts/docker.js`, `server/lib.php`, 7 lang files | 2026.07.11 |
| 199 | folder.view2 migration: imported folders store no `containerImages`/`containerIds`, so rename-tracking didn't work for them until a manual edit+save. `fv3ResolveRenamedContainers()` now backfills those identity maps on the first Docker/VM tab render from live container image / VM UUID data, then persists via the existing `update_ids.php` path. Runs pre-regex-expansion on the raw stored config, so only explicit members are touched and the persisted `containers` list is unchanged (no regex-member freezing). Idempotent — fills only missing entries, so native folders and subsequent loads are no-ops. | `scripts/include/customEvents.js` | 2026.06.25 |
...
| # | Change | File(s) | Version |
|---|--------|---------|---------|
| 225 | Folder editor Regex help (`regex-tooltip`) now documents the `^` catch-all: a folder whose pattern is `^` matches every name and collects containers not explicitly assigned to, or matched by, another folder (place it last) — surfaces the #46/#219 explicit-beats-regex semantics in the UI. Updated all 7 language files + the Folder.page pre-i18n fallback. | `langs/{en,de,es,fr,it,pl,zh}.json`, `Folder.page` | 2026.07.13 |
| 224 | Anti-FOUC (no flash of the plain container/VM list before folders appear): each `.page` injects an inline `<style>` on first paint that hides the native list tbody (`#docker_list` / `#kvm_list` / `#docker_view` / `#vm_view`) while `<html>` lacks a `fv3-*-ready` class, plus an inline 8s `setTimeout` failsafe that reveals regardless. Two `folderEvents` listeners in `customEvents.js` add `fv3-docker-ready` / `fv3-vm-ready` to `<html>` on `docker`/`vm-post-folders-creation`, revealing the now-foldered list. Eliminates the ~300 ms visible flash of Unraid's un-foldered flat list before FolderView3 reorganises it. Safe by construction: the class is added once and never removed, so the hide rule is inert after first paint (no effect on stop/start status, SSE stats, "Started only", or any re-render); `visibility:hidden` (not `display:none`) reserves height so there is no reflow jump and keeps the thead + toolbar visible; the failsafe lives in the `.page`, not the JS bundle, so a broken/absent FolderView3 script still reveals in 8s (never a permanent blank). No CSS variables used (7.0-safe); hide selectors reuse the exact IDs FolderView3 already depends on (native DOM contract stable 7.0→7.3). Live-validated on Unraid 7.3.2 across all three tabs; measured flash window ≈283–306 ms → 0 ms visible. | `folder.view3.Docker.page`, `folder.view3.VMs.page`, `folder.view3.Dashboard.page`, `scripts/include/customEvents.js` | 2026.07.13 |
| 181 | Debug capture: added `env.activePlugins` — a deduped, sorted set of plugin directory names extracted from every `<script src>` and `<link href>` on the page, so "which plugins are on this page (and could affect the view)" is answerable at a glance. Closes a blind spot in `foreignPluginScripts`, whose 20-item cap can be fully consumed by Unraid's own ~20 `dynamix.my.servers` Vue component scripts, truncating a late third-party script; the deduped name-set can't be truncated that way. Diagnostics only. | `scripts/shared.js` | 2026.06.22 |
| 179 | Refinements to the #178 debug capture after reading a real 2026.06.16.1 capture. (1) `foreignInlineStyles` match regex tightened to require a docker-table-specific selector (`docker_containers`/`table-layout`/`folder-preview`/`.folder`/`colspan`) — the previous bare `width:`/`nth-child` alternatives produced misleading `touchesTable` hits on unrelated libraries (Vaul drawer, readmore.js). (2) `tableGeom.cellOverflows` entries now include `overflowX` + a `clipped` flag, so a cell whose content is contained by `overflow:hidden` (e.g. the zeroed Uptime header, scrollWidth 6 but not leaking) isn't misread as a live overflow. Diagnostics only. | `scripts/shared.js` | 2026.06.22 |
| 178 | Self-diagnosing debug capture additions, so a single JSON pins a layout fault without console back-and-forth. (1) `console.warn`/`console.error` tap → trace buffer (armed-only) — surfaces non-fatal errors logged by Unraid/other plugins, not just uncaught exceptions. (2) `env.tableGeom.overflow` — UNROUNDED table/container widths, container `scrollWidth`/`scrollLeftMax` (the Firefox-scrollbar tell), `<th>` sum + deltas; the previously rounded width fields hid sub-pixel overflow entirely. (3) `env.tableGeom.cellOverflows` — auto-names any cell where `scrollWidth > clientWidth` with its padding/box-sizing (the phantom-scrollbar / preview-collapse culprit finder). (4) `env.colspanSelfTest` — transiently pins the folder preview colspan cell to the sum of its spanned `<th>` and measures the preview before/after, then restores, so a broken capture self-proves whether the colspan fix helps (`fixWouldHelp`) — validates the unreproducible fractional-DPR preview collapse from the affected user's own machine. (5) `env.foreignInlineStyles` — inline `<style>` blocks touching the table (the inline-CSS blind spot `foreignStylesheets` missed). (6) `env.globals` — jQuery/Chart/moment/i18n/FV3-fn presence + versions (missing-dependency tell). (7) `perf.resources[].status` — HTTP status to flag 404s. Diagnostics only; no change to rendering when debug is off. | `scripts/shared.js` | 2026.06.22 |
| 174 | Deeper structural diagnostics for an unresolved "preview pushed right / truncated" report (all-browser, reproduces only on one user's setup; ruled out: icon-load race [widthFixDiff=0], generated CSS [colors only], leftover FolderView2 [no footprint], browser engine [Chrome repro of their exact column widths renders correctly]). The capture's `env.foldersRendered` now includes per-row `cells[]` (every child cell's index/class/colSpan/left/width/display), `storageRows` count, and for the first folder `skeletonHTML` (row markup with `.folder-storage` emptied) + `firstStorageRowCells` — to explain why the colspan preview cell lands at offsetLeft≈1100 with an unexplained gap while the ct-name cell is only 220px (vs 332px column). Added `env.tableGeom` (table computed width/min-width/tableLayout, offset/client width, 5-level ancestor width chain, nested-table count) and uncaught `window.error`/`unhandledrejection` logging into the trace buffer. Diagnostics only. | `scripts/shared.js` | 2026.06.22 |
| 172 | Page-load timing in the debug capture. (1) `window.fv3Milestones` (`env.milestones`) — an eviction-proof, name-keyed `{first,last,count}` of `performance.now()` for createFolders-start, folderReq-resolved, createFolders-end, width-fix runs, fonts-ready, first-stat. Separate from the verbose ring buffer, which was evicting the early timeline (300 createFolder lines pushed out the fonts/width-fix #1 markers). (2) `window.fv3IconLoads` (`env.iconLoads`) — per-preview-icon finish timestamps via load/error listeners attached in `fv3SetupPreviewMode`, so icon-finish vs width-fix-run ordering (the load-order race signal) is recoverable. (3) `env.perf` — `getEntriesByType('navigation')` milestones + `resource` timings filtered to plugin assets and preview-icon hosts (real network load times). (4) Trace ring buffer cap raised 300→600. Diagnostics only. | `scripts/shared.js`, `scripts/docker.js` | 2026.06.22 |
| 171 | Broaden debug coverage beyond the Docker width-fix hypothesis. (1) Geometry capture generalized from hardcoded `#docker_containers`-first-folder to all container tables present (`#docker_containers`, `#kvm_table`) plus Dashboard showcase (`.folder-showcase-outer`), capturing per-column thead geometry per table. (2) New `env.foldersRendered` per-folder manifest across every rendered folder (row + showcase): id/name, preview cell placement, the overflow mode actually applied (className), and child/wrapper/divider counts — so "preview pushed right / clipped / wrong content" is diagnosable on any page for every folder. (3) Live-stats diagnostics: `window.fv3StatsState` (transport websocket/sse, lastStatAt, statCount, fallbacks) instrumented in the WebSocket handlers + the Docker SSE path, surfaced in `env.renderSignals.stats` with `apiAvailable`/`cpuCores`/`wsReadyState`/`listeners` — covers the "graphs blank/frozen" class. (4) `fv3CollectCssDebug` now fetches the rendered contents (truncated) of generated/custom CSS loaded from `/boot/config/plugins/folder.view3/` and lists custom scripts — a bad generated rule or community custom-CSS override is no longer invisible. Diagnostics only. | `scripts/shared.js`, `scripts/docker.js` | 2026.06.22 |
| 170 | Debug capture overhaul. (1) Env was collected *before* folders rendered (inside `createFolders` ahead of the draw loop and the width-fix), so every `tr.folder` sample was always null and the layout state was never captured — root cause of an undiagnosable, machine-specific "preview pushed right" report. Capture is now on-demand: each page stashes its data payload by source into `window.fv3DebugPayloads`; `fv3CaptureDebug(source)` downloads it with a *fresh* `fv3CollectEnv()` at click time (post-render). (2) New centered, draggable "FV3 Debug" pill, injected only while debug mode is armed (`fv3debug` keyword) — zero footprint for normal users; `fv3SetDebugPill()` shows/removes it on toggle. (3) Timestamped ring buffer (`fv3TraceBuffer`, cap 300) records the render timeline (existing `fv3Debug` seam calls route through it) plus a `document.fonts.ready` marker — so load-order races are visible. (4) `fv3CollectEnv` now captures per-column thead geometry, `table-layout`, `#fv3-width-style`, exposed width snapshots, the first folder's preview-cell/`.folder-preview` computed alignment, and browser-divergence signals (per-icon `complete`/`naturalWidth`/isSVG, `fonts.status`, scrollbar gutter, DPR, real Unraid release). (5) Width-fix instrumented with a run counter, `fv3GetWidthState()`, and `fv3WidthFixDiff()` (re-measures and diffs vs locked widths — a non-zero delta proves a stale measure-before-load layout). (6) `fv3SanitizeContainersInfo` switched from a denylist to an allowlist — fixes a host-path leak (volume Binds/Mounts, ZFS mountpoints, log/template paths shipped in debug files) and cuts file size ~10x. Diagnostics only; no change to rendering when debug is off. | `scripts/shared.js`, `scripts/docker.js`, `scripts/vm.js`, `scripts/dashboard.js` | 2026.06.22 |
...
| # | Change | File(s) | Version |
|---|--------|---------|---------|
| 231 | `syncContainerOrder`'s stale-entry prune now runs only on a complete, fully-named container list (same #214 guard `readInfo` got in 2026.07.08): a degraded Docker API read (empty or partially-named list) during a folder save / order re-sync / reset-sorting / settings import previously unset every autostart entry and rewrote `/var/lib/docker/unraid-autostart` near-empty, permanently erasing autostart flags and start delays. The reorder rewrite still runs; only the deletion pass is gated. Pre-existing on main (this site had only received `LOCK_EX` in 2026.07.03.2). Found by the pre-stable adversarial review. | `server/lib.php` | 2026.07.17 |
| 230 | Docker/VM tabs: ported #221's dashboard per-folder try/catch to both `createFolders` loops in `docker.js` and `vm.js` — one folder whose config throws in `createFolder()` (e.g. a hand-edited entry missing `settings`) no longer aborts the whole render before the anti-FOUC (#224) reveal event, which hid the ENTIRE native container/VM list for 8s on every page load. The failing folder gets a console.error + persistent banner naming it; the rest render and the list reveals normally. The caller catches (docker.js patched-listview, vm.js ajaxPrefilter) now also add the `fv3-*-ready` class and log a console.error (vm.js's catch was fully silent), so a global load failure reveals the intact native list immediately instead of after the failsafe. | `scripts/docker.js`, `scripts/vm.js` | 2026.07.17 |
| 229 | Dashboard: the #221 per-section catches now add the section's `fv3-docker-ready`/`fv3-vm-ready` class — a transient data-load failure (e.g. `read_order.php` 500) previously banner'd but left the fully-working native tiles `visibility:hidden` until #224's 8s failsafe, with no retry. Found by the pre-stable adversarial review. | `scripts/dashboard.js` | 2026.07.17 |
| 228 | The #219 label-claim union required the `folder.view3` label to be truthy before the folder-name Set membership test: a folder object missing `name` put `undefined` in the Set, and every unlabeled container's `Labels?.['folder.view3']` lookup (also `undefined`) then matched via `Set.has(undefined)` — sweeping all unlabeled containers into `fv3AssignedElsewhere` and silently emptying every regex folder's matches on the Docker tab and Dashboard docker section. Only reachable via hand-edited/imported configs (the editor requires a name). | `scripts/docker.js`, `scripts/dashboard.js` | 2026.07.17 |
| 227 | Folder editor icon preview: #226's `endsWith('/question.png')` loop guard mistook any failing user icon URL that happens to end in `/question.png` (e.g. a remote copy) for the fallback already being shown, leaving the broken-image glyph. The guard now compares the full fallback path, staying loop-proof while letting lookalike URLs fall back. | `Folder.page` | 2026.07.17 |
| 226 | A missing fallback icon (`question.png`) caused an endless 404 retry loop: every FV3 `<img onerror>` reassigned the same fallback src, which re-fired `onerror` (~150 req/s measured with 15 affected imgs; Unraid deletes its own question.png when an icon-less container is removed/updated with cache flags — unraid/webgui#2691). All 10 script-template fallbacks now guard with `this.onerror=null` (fail once, stop); `Folder.page`'s editor preview instead skips the reassign when already showing the fallback, so `updateIcon()` retries keep falling back. Empty icons (`folder.icon`, `el.Icon`, `net.unraid.docker.icon` label) are defaulted to the fallback at render time, so icon-less folders/containers no longer fire an error on every render. Native container rows absorbed into folders still carry Unraid's own unguarded onerror (upstream bug). | `scripts/docker.js`, `scripts/vm.js`, `scripts/dashboard.js`, `scripts/folder.js`, `scripts/folderview3.js`, `scripts/advanced-preview.js`, `Folder.page` | 2026.07.14 |
| 223 | Folder editor: the regex match list (and the custom-action container list it feeds) showed containers label-claimed by another folder as regex-captured, contradicting #219's render semantics where label claims always win. `updateRegex()` now skips candidates whose `folder.view3` label names an existing folder (names snapshotted at editor load into `labelFolderNames`); such containers stay in the pickable list instead. VM editors unaffected (no labels). Found by the post-ship adversarial review of 2026.07.11.2. | `scripts/folder.js` | 2026.07.12 |
| 222 | Dashboard: a failed `read.php`/`read_info.php` load produced two stacked non-dismissable error banners — the request's own `.fail()` banner plus the new (#221) section-catch banner for the same rejection. The `.fail()` handlers now tag the jqXHR (`fv3Bannered`) and the section catches skip re-bannering tagged errors; `read_order.php`/`read_unraid_order.php` failures (previously silent) still get the section banner. Found by the post-ship adversarial review of 2026.07.11.2. | `scripts/dashboard.js` | 2026.07.12 |
| 221 | Dashboard: a match-all regex (`^`, `.*`) crashed the entire folder render (issue #47). The dashboard's two regex expansion sites matched `folder-<id>` placeholder entries in the order array (they lacked the `containersInfo[el]`/`vmInfo[el]` filter the Docker/VM tabs have), so `containersInfo['folder-…']` came back undefined and the first `ct.info…`/`ct.uuid` dereference threw — aborting the shared `createFolders()`, vanishing every VM folder, and each silent retry duplicated folder shells and orphaned tiles. Fix: guard trio ported to both sites (`.trim()` gate, live-name filter, dedup) + `Array.isArray(folder.containers)` shape guards, `if (!ct) continue` placed before the order-array splices, per-folder try/catch at all four builder call sites (console error + persistent `fv3ShowBanner` naming the folder; failed folder excluded from `foldersDone`), per-section try/catch so the Docker half can never abort the VM half, and the caller's swallowed exception is now logged. No more silent retry from section failures. | `scripts/dashboard.js` | 2026.07.11 |
| 220 | Folder editor: saving a regex folder permanently baked its current regex matches into the persisted `containers` array — regex rows render as checked+disabled and jQuery's `:checked` matches disabled inputs, so every save snapshotted them (membership snowballed and stale members survived regex edits). The three persistence collectors (`containers`, `containerIds`, `containerImages`) and the custom-action dialog list now use `:checked:not(:disabled)`; the dialog also stops double-listing regex members (it already adds `selectedRegex` separately). Previously-baked members are unaffected (they render as enabled explicit rows) and can be unticked manually. | `scripts/folder.js` | 2026.07.11 |
| 216 | CSS Tool config-import warning corrected. It claimed imported `url()` references "will be stripped on save for security", but no save path strips `url()` — both `updateCssConfig` (lib.php:926-928) and `generateCssFile`/`sanitizeCustom` (lib.php:972-982) strip only `@import`/`expression(`/`javascript:`; `url()` is intentionally kept so community themes and auto-updates keep working. The note now reads "url() references are kept as-is — verify they point to sources you trust before saving", matching the live inline textarea warning (`#fv3-css-url-warning`) and the READMEs. Message-only change (inline English, not i18n-keyed). | `scripts/csstool.js` | 2026.07.08 |
| 215 | `fv3_atomic_write` hardened (config-integrity): (a) unique per-process temp name (`.tmp.<pid>.<uniqid>`) so two concurrent writers no longer share and corrupt one `.tmp`; (b) the temp is unlinked on *every* failure path (write-fail and rename-fail) — under the old fixed name a failure left one bounded stale file, but unique names turned that into an unbounded per-write orphan (worst on the boot flash the writes target); (c) a short/partial write is now failure (`!== strlen($data)`, was `=== false`) so a truncated write (flash ENOSPC) can't be renamed over a good config; (d) a non-string payload is rejected up front (`is_string`), so a `json_encode()`/`file_get_contents()` returning `false` no longer coerces to `''` (the `string $data` param) and atomically blanks a valid config. Found by the pre-push adversarial verify (rounds 1–2). Companion to #21. | `server/lib.php` | 2026.07.08 |
| 214 | `readInfo()` stale-autostart cleanup no longer wipes or curtails `/var/lib/docker/unraid-autostart` on a degraded Docker read. The first guard (`!empty($allCtNames)`) tested only array-non-emptiness, so a non-array `$cts` (socket blip) OR a container list whose entries lacked `Names[0]` (yielding `['','']`) still passed, and `array_filter` then dropped every line → empty file — the exact data loss the guard targeted. Now `$cts` is normalised to an array once (`if (!is_array($cts)) $cts = [];`, which also fixes the later unguarded `foreach ($cts)` in readInfo), and the prune runs only on a complete, fully-named list (`!empty($ctNames) && !in_array('', $ctNames, true)`) with strict `in_array`. Added `FILE_SKIP_EMPTY_LINES` (matching the sibling prune in `syncContainerOrder`) so a blank line no longer fires a spurious rewrite of the system file plus a false "removed N stale entries" log. Found/refined by the pre-push adversarial verify. Second of two prune sites; the `syncContainerOrder` one was locked in 2026.07.03.2. | `server/lib.php` | 2026.07.08 |
| 213 | Packaging: switched to standard Slackware naming `folder.view3-<version>-x86_64-1.txz` so `upgradepkg --install-new` matches across versions, and removed the pre-install `removepkg` of the live package (a failed download previously left the plugin uninstalled). Cleanup of stale package DB entries moved to post-install as metadata-only `rm` (never `removepkg`, which would delete shared files). Matches the appdata.cleanup.ng / user.scripts / tailscale convention; the old→standard transition was verified live on 7.3.1. | `pkg_build.sh`, `folder.view3.plg` | 2026.07.07 |
| 212 | Per-tab import file input is reset after selection so re-selecting the same file re-fires (was a silent no-op, exposed by the new backup-bundle warning). | `scripts/folderview3.js` | 2026.07.07 |
| 211 | Dashboard bulk-action error dialog now shows the real message (`escapeHtml(e.text \|\| …)`) instead of the boolean `false` — the dashboard copies had a stale `.map(e => e.success)` that docker.js/vm.js didn't. Surfaced once the allSettled fix made the dialog reachable. | `scripts/dashboard.js` | 2026.07.07 |
| 210 | Width engine no longer throws a `ReferenceError` at the end of every run — a debug log referenced an undeclared `widthsCollapsed` (the var is `collapsed`). Real work already completed, but the throw aborted the caller's continuation and spammed the console each refresh. | `scripts/shared.js` | 2026.07.07 |
| 209 | User-script custom actions no longer strand the loading spinner: `fv3RunUserScript`'s awaited `$.post`s are wrapped in try/catch, and the Dashboard's two inline copies now call the shared helper — which also fixes the VM copy's `directPHP` branch (it duplicated its `else` and never ran the script directly). | `scripts/shared.js`, `scripts/dashboard.js` | 2026.07.07 |
| 208 | Completed the Dashboard null-guard fix (200/201 hardened only the restore loop): `expandFolderDocker/VM`, the collapse handlers, `addDockerFolderContext/addVMFolderContext`, and `folderDockerCustomAction/folderVMCustomAction` all dereferenced `globalFolders.docker[id]`/`.vms[id]` behind a category-only guard, so a folder added while the Dashboard was open (esp. with expand-on-dashboard) threw a `TypeError` and blanked the whole Dashboard. All sites now null-check the id; `vm.js` `folderCustomAction` got the folder guard docker.js already had. | `scripts/dashboard.js`, `scripts/vm.js` | 2026.07.07 |
| 207 | **Unraid 7.0.0/7.0.1:** `DockerUtil::host()` did not exist before 7.1.0, so `readInfo()` fataled on the Docker AJAX path and no Docker folders rendered. Guarded with `method_exists` + a `$_SERVER['SERVER_ADDR']` fallback. Verified against unraid/webgui tags: absent on 7.0.0/7.0.1, present 7.1.0+. | `server/lib.php` | 2026.07.07 |
| 205 | Delete-folder button no longer uses `!important` — raised selector specificity instead (`.fv3-form-actions-right input.fv3-delete-folder-btn`). Preset chevron hover text made theme-overridable via `--fv3-toggle-hover-text` (default `#fff`). | `styles/folder.css`, `styles/folder-common.css` | 2026.07.07 |
| 204 | Docker "hide update column" folder rows now derive their colspan from the live table (`colspan + 1`) instead of a hardcoded `6`, so they span correctly on Unraid versions whose Docker table isn't 10 columns (7.0+). Identical on 7.2/7.3. | `scripts/docker.js` | 2026.07.07 |
| 203 | Editing a folder created by an older version no longer throws when `settings.preview` is absent (defaults to 1, matching the form's default). | `scripts/folder.js` | 2026.07.07 |
| 202 | Per-tab Docker/VM folder import now rejects a full backup bundle (detected by `fv3_export_version`) with a warning instead of writing the bundle's top-level keys as broken folders. | `scripts/folderview3.js` | 2026.07.07 |
| 201 | Adding a folder while the Dashboard is open no longer throws a TypeError and blanks the Dashboard — the expand-restore guard now null-checks the folder id, not just the docker/vms category (matches docker.js/vm.js). | `scripts/dashboard.js` | 2026.07.07 |
| 200 | Bulk start/stop/restart and custom actions now use `Promise.allSettled`, so a single failed container request no longer leaves the loading spinner stuck and the container list un-refreshed. | `scripts/docker.js`, `scripts/vm.js`, `scripts/dashboard.js` | 2026.07.07 |
| 199 | Unguarded Tailscale `funnel` label read no longer emits an "Undefined array key" warning for every non-Tailscale container. | `server/lib.php` | 2026.07.07 |
| 198 | Reworked the Docker table width engine (replaces the 2026.06.22 scroll-on-expand behaviour). Columns fit the viewport and shrink the flexible middle columns to a floor before scrolling, so the table scrolls only when content genuinely can't fit — never crushes. Collapsed snapshot is derived from expanded so expanding a folder shifts only Volume (no rearranging the middle columns). Loadlist refresh and view toggle replay the cached per-view snapshot instead of re-measuring (no Volume snap on refresh, no a→b→a drift). View toggle uses a FLIP animation off the switch's change event. Uptime hidden when all folders are collapsed; shown on expand or with a loose container. Firefox: clip the Uptime cell's nowrap content (phantom scrollbar) and exclude the table from scroll anchoring (toggle-animation warning). Also trimmed verbose code comments repo-wide. | `scripts/shared.js`, `scripts/docker.js`, `styles/docker.css`, plus comment-only trims in `dashboard.js`/`vm.js`/`folder.js`/`folderview3.js` | 2026.06.23 |
| 197 | Containers using `container:xxx` network mode (sharing another container's network namespace — e.g. a service routed through a Gluetun VPN sidecar) showed no WebUI globe icon (reported on the forum by HHUBS for qbittorrent + arr stacks behind GluetunVPN). Root cause: in `readInfo()`, `[IP]`/`[PORT:]` WebUI templates are resolved against the container's own IP/port bindings, but a `container:` child has no network of its own — its `NetworkSettings.Ports` is `{}` and the IP/published ports live on the **parent** container — so the code blanked the WebUI URL outright for any `container:`/`none` mode, giving an empty `State.WebUi` and no icon. folder.view2 still showed it because it resolved more naively against the host IP. Fix: for `container:xxx` mode, look up the parent container (named in the mode string) via `getContainerDetails()` and resolve `[PORT:]` against the parent's published `HostConfig.PortBindings`, with `[IP]` as the Unraid host — the WebUI link now renders. `none` mode still yields no WebUI (no reachable interface). Verified live against the parent-port lookup: a `[PORT:5010]` template on a `container:`-mode child (Mousehole, parent qbittorrent) resolves to `http://192.168.2.206:5010`. | `server/lib.php` | 2026.06.22 |
| 196 | Container CPU% differed by stats transport: the SSE fallback divided docker's CPU% by core count (`divideByCore=true`) while the GraphQL WebSocket path used it raw (`divideByCore=false`), even though both deliver the same docker-native value (verified live: Unraid's API computes `cpuPercent` via the `systeminformation` lib formula `(cpuDelta/systemDelta)*online_cpus*100`, byte-identical to `docker stats CPUPerc`). Result: on the API path a folder's CPU read up to ~Ncores× too high and the CPU bar (`Math.min(100, loadCpu)`) pinned at 100% for any container using more than one core, and the value jumped when the API dropped to SSE. Fix: removed the `divideByCore` parameter and normalize unconditionally (`cpuVal / cpus`) inside `fv3UpdateFolderStats`, and divide the WS path in the advanced-preview CPU graph to match — both transports now produce identical, correctly-scaled CPU. | `scripts/docker.js`, `scripts/advanced-preview.js` | 2026.06.22 |
| 195 | Mobile: the advanced-view container popup overflowed off the right edge of the screen (reported with screenshot, reproduced live at 430px). Root-cause rule: `.fv3-mobile-details > .action-info, .fv3-mobile-details > .info-section` set `padding: 8px` + a border on panes already at `width: 100%`, but with default `box-sizing: content-box` the padding/border stacked on top of 100% — making `.action-info` 393px inside a 375px parent (~18px off-screen) and the popup's content `scrollWidth` exceed its box. Fix: `box-sizing: border-box` on that rule so the padding sits inside the 100% width. Mirrored in both `docker.css` and `dashboard.css` (parity check passes). Verified live (mobile 430px): action pane 375px / right 424 in-view, graph canvas 361px in-view, popup content no longer overflows, page does not scroll horizontally. | `styles/docker.css`, `styles/dashboard.css` | 2026.06.22 |
| 194 | Incognito mode deleted the icons on container-popup / menu links whose URL contains the container or image name — "Project Page" and "More Info" turned into a plain "WebUI" label with no icon (Support/Donate survived because their URLs don't contain the name). Reported with screenshots, root-caused live. Cause: the link scrubber ran `el.textContent = 'WebUI'` on any matched `a[href*="://"]`, which replaces all child nodes including the `<i>` icon. Fix: preserve the icon and label — only neutralise the href (`#`) and scrub the name from the link's text nodes via `scrubTextNodes` (so a hostname-based WebUI link still loses the name while menu links keep their label + icon). Applied to both the page-wide scrubber (`fv3IncognitoApply`) and the advanced-preview popup scrubber (`fv3IncognitoScrubTooltip`); restore now keys off `data-fv3-real-href` / `data-fv3-scrubbed`. Verified live (synthetic + restore): icons preserved through scrub and toggle-off, names still hidden. | `scripts/shared.js` | 2026.06.22 |
| 193 | Folder preview icons shifted ~4px sideways when another folder was expanded/collapsed (a regression from #189's per-view cache, diagnosed live via Chrome MCP). Root cause: the collapsed and expanded snapshots are now measured in separate runs, and the version-column (`verCap`) intrinsic measurement comes out a few px different between the collapsed-state and expanded-state runs — so the **anchor columns left of the preview** (Application=0, Version=1) diverged between snapshots, moving the preview cell's left edge (and its pills) when the table switched layouts on expand. Confirmed against main (preview cell swings 187px there yet pills stay pinned, range 0) — main measures both states in one pass so the anchors never diverge. Fix: after building both snapshots, pin the anchor columns to the same (`max`) width in both, so the preview's left edge is byte-identical collapsed vs expanded; `max()` never clips (verCap already ellipsises overflow). Verified live: pills 378→378→378 (range 0) across expand/collapse, matching main, with autostart/CPU/collapsed-fit/toggle-determinism all still correct. | `scripts/shared.js` | 2026.06.22 |
| 192 | At browser zoom ≠ 100% (or 125%/150% display scaling), expanding a folder — or toggling view while expanded — forced the Uptime column back into the viewport with no horizontal scroll (diagnosed live via Chrome MCP; reproduced by overriding `devicePixelRatio` to a fractional value). Root cause: at fractional DPR the table falls back to `table-layout:auto` (the pre-existing #? DPR fix, because `fixed` breaks the preview colspan cell at fractional scaling), where the per-`<th>` snapshot widths are only *hints* — and because the cells are wrappable (`white-space:normal`) the browser shrinks the whole table to fit the container instead of scrolling. Fix: `fv3ApplyCachedWidths` now pins the table's `min-width` to the snapshot total, so the table reaches its real width and the `.TableContainer` scrolls under both layout modes; the pin is cleared before each measurement so it can't skew sizing. Verified live at simulated DPR 1.5: collapsed fits, expanded scrolls (1867 in a 1682 container), toggle-while-expanded scrolls; integer DPR unchanged. | `scripts/shared.js` | 2026.06.22 |
| 191 | In Advanced view, expanding a folder collapsed the CPU/Memory column to 0px so its stats spilled into the Autostart column (reported with screenshot, reproduced live). Root cause: the expanded width snapshot was cached **globally** (one for both views); it was first captured during a Basic-view expansion where the CPU column is hidden (width 0), then replayed in Advanced view. Fix: the snapshot cache is now **per-view** (`_fv3ViewCache` keyed by `fv3CurrentDockerView()`), so Basic and advanced each keep their own collapsed+expanded snapshots and advanced-expanded sizes the CPU/Memory column from its own measurement. Verified live: CPU column 166–173px, right edge flush with Autostart's left (no bleed), stable across repeat expand/collapse. | `scripts/shared.js` | 2026.06.22 |
| 190 | Toggling Basic→Advanced→Basic left the table sized differently from a fresh Basic load — the CPU column kept a 158px reservation (crushing Volume, shoving Autostart off-screen) and the middle columns redistributed (diagnosed live via Chrome MCP; snapshots diffed fresh vs round-trip). Two causes. (1) The re-measure on toggle ran before Unraid finished hiding the advanced CPU column, so it measured CPU as still-visible and baked its width into the Basic snapshot — fixed by detecting advanced-only columns from the view **cookie** (`fv3CurrentDockerView()`), authoritative the instant the toggle registers, rather than the column's transient computed `display`. (2) Unraid's `listview()`/`readmore` keep re-processing the cells for ~600ms after a toggle, so the ~250ms re-measure captured transient middle-column widths that never settled back — fixed by a second settle re-measure ~700ms after the toggle (empirically settles by ~600ms). Verified live: basic↔advanced↔basic now returns to byte-identical collapsed and expanded snapshots. | `scripts/shared.js`, `scripts/docker.js` | 2026.06.22 |
| 189 | Autostart/Uptime were pushed ~half off-screen on the first expand after a page load and stayed there until an unrelated resize "snapped" them back (reported, reproduced live). Root cause: the expanded column widths are only accurate when measured with container rows genuinely visible — the all-collapsed "move-out" measurement over-estimates the middle columns by ~48px (autostart offBy 56 vs 15). Fix: the **first genuine expansion in each view re-measures once** to capture an accurate expanded snapshot, which is then cached and replayed on all later expand/collapse (no per-toggle re-measure → no preview jitter). Combined with a post-load settle re-measure (1200/3000ms, only while all folders are collapsed) to refresh the cache after icons/stats settle. Verified live: autostart lands at offBy 15 on first expand and stays, `runs` constant across repeat expand/collapse. | `scripts/shared.js`, `scripts/docker.js` | 2026.06.22 |
| 188 | Re-introduces native scroll-on-expand for the Docker table (the intent of #184, which #187 reverted because it jumped) — done correctly this time. Two-mode `distributeSlack`: the collapsed/folders-only snapshot **fits the viewport** (safe — the preview colspan wraps, no per-cell data to overlap), while the expanded snapshot keeps **content widths clamped to ≥ the collapsed width**, so expanding only ever grows/holds a column (never shrinks one) — the table extends rightward and the `.TableContainer` scrolls, instead of compacting columns leftward (which caused the chevron/heading jump) or crushing them into overlap at narrow widths. Verified live via Chrome MCP across basic/advanced × collapsed/expanded: collapsed fits (no scroll), expand scrolls right with zero column overlap and zero leftward shift, chevron range 0; page itself never scrolls (only the table container does, like native Unraid). VM page left untouched — it already uses native `auto` layout and behaves correctly. | `scripts/shared.js` | 2026.06.22 |
| 187 | Basic-view table instability when toggling Basic↔Advanced and expanding/collapsing folders (diagnosed live via Chrome MCP). Two independent causes, both fixed. (1) `verCap` (the update-column cap) was measured non-deterministically: a block element in the version cell was measured at its rendered *column* width, a feedback that flip-flopped the version column between two widths (e.g. 131↔185) run-to-run and shifted the headings. Now every version-cell child is measured at its **intrinsic** width — force `inline-block` for hidden *or* block-ish elements, not only `display:none` ones — so verCap is deterministic and still covers the advanced "force update" content (the #182 / 2026.06.18.1 truncation fix is retained). (2) the 2026.06.18.2 two-mode `distributeSlack` ('natural' for the expanded snapshot) kept the expanded table at content width and scrolling while the collapsed snapshot fit the window, so the table width swung ~266px and flashed a horizontal scrollbar on **every** expand/collapse; reverted to main's single fit mode (`widthsCollapsed` derived from `widthsExpanded`) so both snapshots fill the same width. Verified live: deterministic verCap across repeated runs and view toggles, and constant table width on expand/collapse. Tradeoff (accepted): the narrow-window column overlap that #184 addressed returns to main's behaviour. Separately confirmed the slow-toggle chevron shift is a pre-existing `table-layout:fixed` quirk present on main too (deployed main's `shared.js` and reproduced it), so it is left as-is; pinning the table width to fix it was tried and rejected (it left a gap at the window edge). | `scripts/shared.js` | 2026.06.22 |
| 186 | Revert of #185 (2026.06.18.3). #185 added an explicit cell-`scrollWidth` content measurement on the belief that #184 (2026.06.18.2) hadn't taken effect — but that belief came from a **flawed verification**: constraining the table's immediate container with `max-width` also caps the table itself, manufacturing a crush that does not occur in a real narrow window. Re-verified faithfully (narrowing the page top-down + screenshot on the deployed .2): under auto-layout the table expands to its content inside the `overflow-x:auto` container, so #184's two-mode `distributeSlack` ('natural' for the expanded snapshot) already keeps columns at content width and scrolls the table — zero overlap. The #185 measurement was therefore redundant and carried a small risk of widening columns at wide widths, so it is removed and the width-fix returns to the verified #184 behaviour. | `scripts/shared.js` | 2026.06.22 |
| 185 | **Superseded by #186 (reverted in 2026.06.18.4).** Completes #184 (2026.06.18.2), which did not take effect. The expanded snapshot kept columns at the widths measured during the width-fix's auto-layout pass, but that pass runs while the table is constrained to its container, so at narrow widths the measured widths were already the squeezed values — `distributeSlack`'s 'natural' mode faithfully kept them and still crushed. Fix: measure true (unwrapped) column content via each container-row cell's `scrollWidth` (container-independent) and feed it into `maxCols`, for both the moved-in storage rows and any already-expanded rows (autostart cap and verCap override still apply afterward). Now the expanded snapshot keeps real content widths and the table scrolls. Verified live via a container-width simulation: Network/IP/Port/LAN/Volume sized to content (138/174/119/189/214), table 1648px inside an 1100px container → scrolls, zero middle-cell overflow. | `scripts/shared.js` | 2026.06.22 |
| 184 | **Reverted in 2026.06.19.1 (see #187) — the scroll mode caused a table-width jump on every expand/collapse.** At narrow window widths (e.g. browser < ~1300px, or a side panel open), an expanded folder's container rows had their middle columns (Network, Container IP, Port, LAN, Volume) crushed to ~50px and the un-clipped native cell content overflowed and overlapped — "columns blend into one", including the Volume cell collapsing to a single vertical character. Root cause: the width-fix's `distributeSlack` forces the table to exactly fill the container by scaling the flexible middle columns down (toward a 40px floor), and a final exact-fit nudge then subtracted the remaining overflow from the Volume column. Diagnosed live (Chrome MCP): verCap was correct (103) and the column widths summed to the container — the columns were simply crushed below their content; not caused by the 2026.06.18.1 verCap change (proven inert in advanced view). Fix: split the snapshot into two modes — the collapsed/folders-only snapshot still fits the window (safe: the preview spans those columns), but the expanded snapshot keeps columns at their content width and lets the `TableContainer` (`overflow-x:auto`) scroll horizontally when they don't fit, instead of crushing them (native Unraid behaviour). Verified live that the table scrolls under `table-layout:fixed`, and offline that the expanded snapshot keeps content widths at narrow `targetW` while wide and collapsed snapshots are unchanged. | `scripts/shared.js` | 2026.06.22 |
| 183 | `ResizeObserver loop completed with undelivered notifications` flooded the debug trace as `level:error` (a burst of ~20 during initial load). Diagnosed live (Chrome MCP) as benign, intermittent, load-only settling from the preview-mode observers (clip / expand / scroll in `fv3SetupPreviewMode`) re-firing as container icons stream in — not an exception, self-converging in a few hundred ms, and absent on idle / toggle / resize / expand (0 errors measured in all four). The pill-sizing observer is inactive without a CSS preset, so it is not the source here. Fix (diagnostics + minor): `window.onerror` now filters this benign message and counts it (`window._fv3ROLoopCount`, surfaced as `renderSignals.roLoopSettles`) instead of logging each as an error, so real failures aren't masked; and the pill-sizing `ResizeObserver` routes through the existing 50ms debounce (consistent with every other pill trigger, verified to coalesce 20 rapid fires → 1 pass) instead of resizing inside its own delivery cycle. The clip/expand/scroll preview observers were deliberately left untouched. | `scripts/shared.js` | 2026.06.22 |
| 182 | Update column truncated with an ellipsis after toggling Basic→Advanced view with a folder expanded — "force update" / version text clipped until a later interaction "popped" it back. Root-caused live (Chrome MCP) to a timing race in the width-fix `verCap` measurement: it measures the version-cell content of rows parked in `.folder-storage`, but on a Basic→Advanced toggle a width-fix can fire before Unraid's `listview()` reveals the advanced "force update" content (still `display:none`), so it computes a too-small cap and clips the now-visible advanced text until a later run corrects it. Reproduced the exact race live: shipped code → cap 86px / 9 truncations (matching the user's debug capture); the straggler run recovered to 103px. Fix: force-show hidden version-cell children during measurement (after the existing cap-style removal) so `verCap` fits the advanced content regardless of when the fix fires — timing-independent. Verified live: fixed → cap 103px / 0 truncations, with the page restored clean. | `scripts/shared.js` | 2026.06.22 |
| 180 | "Only icon" preview: clicking a container's WebUI / console / logs action button opened the per-container dialog instead of performing the action, and the WebUI globe never opened the WebUI. Root-caused live (Chrome MCP): in `Only icon` mode the preview clones the bare `span.hand`, which carries Unraid's `addDockerContainerContext` (Docker) / `addVMFolderContext` (VM) onclick; the action buttons are appended *inside* that span, so every button click bubbled up and fired the dialog. The console/logs buttons called `event.preventDefault()` but not `event.stopPropagation()` (dialog opened on top of the terminal); the WebUI `<a target="_blank">` had no handler at all, so the bubbled `addDockerContainerContext` (which `preventDefault`s internally) cancelled the new-tab navigation. Other preview modes append into `span.inner` (no onclick) and were unaffected. Fix: added `event.stopPropagation()` to all five buttons (docker.js webui/console/logs, vm.js console/logs), plus the missing `event.preventDefault()` on the vm.js logs button. Verified live on the affected folder: dialog no longer fires on any button and the WebUI link is no longer cancelled. | `scripts/docker.js`, `scripts/vm.js` | 2026.06.22 |
| 179 | Compose / 3rd-party container autostart label wrapped to two lines on narrower windows. These containers render `fa-docker + "Compose"/"3rd Party"` in the autostart column (no toggle); the width-fix locks that column near ~100px, leaving ~74px usable after Unraid's `6px/20px` padding, and with default `white-space:normal` the `icon + " " + "3rd Party"` (~95px) breaks "Party" to a second line. Diagnosed live (Chrome MCP): the icon's apparent gap is the standard `fa-fw` fixed-width centering + text space, not a stray element; the content fits the locked column exactly at `white-space:nowrap` (`scrollWidth == clientWidth == 100`, no clip; line boxes 2 → 1). Fix: CSS rule on the autostart column (`#docker_containers > tbody > tr > td:nth-last-child(2)`, colspan-safe trailing column) sets `white-space:nowrap` + `overflow:hidden` (the latter guards against a longer label re-introducing the horizontal scrollbar from #177). CSS-only — does not touch the regression-prone width-fix JS. | `styles/docker.css` | 2026.06.22 |
| 177 | Firefox-only phantom horizontal scrollbar on the Docker containers table (basic + advanced view; Chrome unaffected). Root-caused live to the width-fix: when it collapses the trailing Uptime column to width 0 it zeroes the `<th>` padding/font-size, but (a) the body `<td>` cells in that column kept their content-box horizontal padding (6px + 20px = 26px), which overflows the zero-width column under `table-layout:fixed`, and (b) the zeroed header still had a ~6px sort-arrow/inner-span sliver (font-size:0 doesn't shrink it). Firefox renders a scrollbar for that fractional `scrollWidth` overflow; Chrome discards it (and FV2 never pinned `<th>` widths, so never triggered it). Diagnostic signature: `scrollWidth > clientWidth` while no element's rect crosses the boundary (in-cell padding overflow) + `scrollLeftMax > 0` (Firefox). Fix in `fv3ApplyCachedWidths`: for each width-0 column, zero the body cells' horizontal padding via a scoped `#fv3-zerocol-style` rule (`td:nth-last-child(headerCount-i)` — colspan-safe, ID-chain specificity, no `!important`) and add `overflow:hidden` to the zeroed `<th>`. Measured end-to-end on the affected Firefox: overflow 26px → 6px → 0. | `scripts/shared.js` | 2026.06.22 |
| 176 | **Correct fix for the fractional-DPR preview collapse — #175 / 2026.06.14.2 was ineffective and is rolled back.** `.2` set explicit `width` on the folder-row `<td>` cells, but under `table-layout:fixed` the browser ignores body-cell widths (columns come from the `<th>`) — confirmed by a follow-up capture where the colspan cell carried `width:1985px` yet rendered `0`. Root cause: `table-layout:fixed` *itself* mis-distributes the colspan preview cell at fractional `devicePixelRatio` (a regression vs FolderView2, which never used fixed layout). Fix on the **right element (the table)**: `fv3ApplyCachedWidths` now sets `table-layout: fixed` only when `Number.isInteger(devicePixelRatio)`; at fractional DPR it falls back to the browser's auto layout (the computed `<th>` width hints still keep columns aligned), which sizes the colspan cell correctly — the same layout mode FV2 shipped. Verified live at integer DPR: keeps `fixed` (no regression), and `auto` also renders the preview correctly (cell spans the middle columns), so the fallback is safe. | `scripts/shared.js` | 2026.06.22 |
| 175 | Folder preview pushed far right and clipped on displays with **fractional `devicePixelRatio`** (e.g. Windows display scaling 125%, or non-100% browser zoom). Root cause confirmed from an affected user's cell-level debug capture (DPR 1.132): the Docker table runs `table-layout:fixed` and each folder row has a `colspan=5` preview cell; at fractional DPR the browser's fixed-layout colspan distribution fails and collapses that cell to its content (~210px) while the neighbouring cells balloon (updatecolumn 880px, autostart 722px), shoving the preview to offsetLeft ~1100. Reproduces on ALL browsers on the affected machine (OS display scaling is system-wide) but never at integer DPR (so it never appeared on the maintainer's DPR-2 Mac, and a Chrome repro of the exact column widths at DPR 2 rendered correctly). `widthFixDiff` was all-zero — the computed column widths were correct; the body cells just didn't honour them. Fix: `fv3ApplyCachedWidths` now sets explicit per-cell widths on every `tr.folder` (walking cells and summing each cell's spanned columns, so the preview td = sum of its 5 columns), overriding the colspan-collapse fallback. Verified a no-op at integer DPR (live, DPR 2: preview cell unchanged at 351/860). Stale folder-cell widths are cleared at the start of the width-fix so re-measurement stays clean. | `scripts/shared.js` | 2026.06.22 |
| 173 | Live CPU/MEM stats always used the SSE fallback even when the GraphQL WebSocket was available. The Docker stats wiring decided WS-vs-SSE with a synchronous `if (fv3ApiAvailable && fv3CpuCores)` at module-parse time, but those globals are populated asynchronously by `fv3DetectApi()` (a /graphql fetch) — so they were always `null` when the check ran and the `else`/SSE branch always won; `fv3ConnectStats` (WebSocket) was never invoked on the Docker tab. Confirmed live via Chrome MCP (wsReadyState=null, transport=sse, apiAvailable=true) and the fix mechanism validated in-page (calling `fv3ConnectStats` post-detection → wsReadyState=1, stat delivered). Fix: wrap the transport decision in `fv3DetectApi().then(...)` so it runs after detection resolves. Low impact (SSE worked); restores the intended WebSocket transport. | `scripts/docker.js` | 2026.06.22 |
...
| 170 | On a failed/timed-out folder data request, `loadedFolder` latched `true` before the un-awaited `createFolders()` ran, leaving folders blank with no retry until manual reload. Set and reset the flag in a `.catch()` (Docker/VM) so a later render recovers; Dashboard now `await`s `createFolders()` before applying layouts (also fixes layouts applying to a half-built DOM on slow loads). | `scripts/docker.js`, `scripts/vm.js`, `scripts/dashboard.js` | 2026.06.22 |
| 171 | With a CSS preset/accent active, container names in a folder preview that turned orange on update were overridden back to the preset colour: the guarded rules at `folder-common.css:207-208` excluded `.orange-text` but an older unguarded sibling at `:169-170` still matched. Added `:not(.orange-text)` to the sibling. | `styles/folder-common.css` | 2026.06.22 |
| 172 | Live CPU/MEM stats died silently when the GraphQL WebSocket closed cleanly (server restart, nginx reload, sleep/wake) — only `onerror` triggered the SSE fallback, not `onclose`. `onclose` now invokes the fallback unless the close was a deliberate teardown (flagged in the `beforeunload` cleanup). | `scripts/shared.js` | 2026.06.22 |
| 173 | Dashboard VM custom action (non-directPHP branch) passed an empty `&arg2=`, dropping configured script arguments; the Docker twin and the VM directPHP branch both pass `+args`. Fixed to match. | `scripts/dashboard.js` | 2026.06.22 |
| 174 | Hardening: top-level `localStorage` read in shared.js wrapped in try/catch so browsers blocking site data don't throw at parse time and take down all folder rendering. Removed dead `getAllPresets()` and `isCssDarkTheme()` helpers. | `scripts/shared.js`, `scripts/csstool.js` | 2026.06.22 |
