...
|---|--------|---------|---------|
| 167 | CSRF was unenforced: `fv3_validate_csrf()` existed but was never called, and `$var` was never populated. All POST endpoints (create/update/delete/themes/CSS/settings) accepted forged cross-site requests from a logged-in admin's browser. Wired `fv3_validate_csrf()` into `fv3_post_init()` and load `/var/local/emhttp/var.ini` when `$var` is unset. Client already sends the token on every mutation (jQuery prefilters + fetch bodies); verified valid/forged/missing handling against live var.ini on PHP 8.4. | `server/lib.php` | 2026.06.13.1 |
