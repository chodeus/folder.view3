...
const populateTable = async () => {
    ...
    try {
        ...
        fv3SwalError(fv3I18nOr('folder-data-load-failed', 'Could not load folder data. Try refreshing the page.'), 'SETTINGS');
        return;
    }
    ...
    for (const id of vmIds) {
        const folder = vms[id];
        const fld = `<tr><td>${escapeHtml(id)}</td><td><img src="${escapeHtml(folder.icon || '/plugins/dynamix.docker.manager/images/question.png')}" class="img" onerror="this.onerror=null;this.src='/plugins/dynamix.docker.manager/images/question.png';">${escapeHtml(folder.name)}</td><td><button title="${escapeHtml(fv3I18nOr('export', 'Export'))}" data-i18n="[title]export" onclick="downloadVm(${fv3JsArg(id)})"><i class="fa fa-download"></i></button><button title="${escapeHtml(fv3I18nOr('delete', 'Delete'))}" data-i18n="[title]delete" onclick="clearVm(${fv3JsArg(id)})"><i class="fa fa-trash"></i></button></td></tr>`;
        vmsTable.append($(fld));
    }
...
const fv3ForeignError = (code) => ({
    ...
    'vm-membership-unavailable': () => fv3I18nOr('foreign-error-vm-membership-unavailable', 'FolderView3 could not read the VM list, so it cannot tell which VMs your folders already hold. Try the import again once the VM service is running.'),
...
const fv3ForeignPreviewHtml = (report) => {
    ...
    for (const [type, r] of Object.entries(report.types || {})) {
        ...
        const counts = [
            [r.merged_children, 'foreign-note-merged', '$1 nested folders merged into their top-level folder (their own name, icon and settings are not kept)'],
            [r.broken_parents, 'foreign-note-broken-parents', '$1 folders had a missing, invalid or looping parent and were kept as top-level'],
            [r.renamed, 'foreign-note-renamed', '$1 folders renamed because the name was missing, not usable, or already in use'],
            ...
            [r.dropped_icons, 'foreign-note-icons', '$1 icons left out because FolderView3 cannot use them'],
            [r.dropped_regex, 'foreign-note-regex', '$1 folder regexes left out because FolderView3 cannot use them or their folder was merged into a top-level folder'],
