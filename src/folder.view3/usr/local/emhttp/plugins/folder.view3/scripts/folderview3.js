...
const populateTable = async () => {
    ...
    for (const id of vmIds) {
        const folder = vms[id];
        const fld = `<tr><td>${escapeHtml(id)}</td><td><img src="${escapeHtml(folder.icon || '/plugins/dynamix.docker.manager/images/question.png')}" class="img" onerror="this.onerror=null;this.src='/plugins/dynamix.docker.manager/images/question.png';">${escapeHtml(folder.name)}</td><td><button title="Export" onclick="downloadVm(${fv3JsArg(id)})"><i class="fa fa-download"></i></button><button title="Delete" onclick="clearVm(${fv3JsArg(id)})"><i class="fa fa-trash"></i></button></td></tr>`;
        vmsTable.append($(fld));
    }
